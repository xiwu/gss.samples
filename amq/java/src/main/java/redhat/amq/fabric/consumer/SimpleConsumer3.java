					 ...
					 try {
						 if (message instanceof TextMessage) {
							 ...
							 if(text.contains("RD")){
								 ...
								 MyRunnable runnable = new SimpleConsumer3.MyRunnable(session,message);
...
//								 
								 if("message1".equalsIgnoreCase(text)) {
							         //如果消息为 message1 ，模拟消息无法正常完成 重复recover
									 Thread.sleep(15000);
									 session.recover();
//									 System.out.println("do nothing:"+text);
//									 
//									 System.out.println("before send ack.");
//									 long oldTime = System.currentTimeMillis();
//									 message.acknowledge();
//									 System.out.println("after send ack,time cost:"+(System.currentTimeMillis()-oldTime)+"ms");
